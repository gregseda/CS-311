...
//ALGORITHM: the loop continously displays each element in the queue, until there are no more elements in the queue
...
void queue::goToBack()
{
  ...
  else if(count > 1){                  // if the counter is greater than 1, it sends the front element to the back of the queue
    remove(temp);
    add(temp);}
}
